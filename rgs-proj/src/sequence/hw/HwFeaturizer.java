...
public class HwFeaturizer extends AbstractFeaturizer { 
	
	private static final long serialVersionUID = 2600327382486754166L;
	
	public static final int HwSingleLetterFeatLen = 128;
	...
	public HwFeaturizer(String[] albt, int sigflen, boolean pairwise, boolean trip, boolean quad) {
		alphabet = albt;
...
/*
	public int computePairIdx(int val1, int val2) {
		return (val1 * alphabet.length + val2);
	}
	
	public int computeTripletIdx(int val1, int val2, int val3) {
		return -1;
	}
	
	public int computeQuadIdx(int val1, int val2, int val3, int val4) {
		return -1;
	}
	
	public String pairToStr(int val1, int val2) {
		return (String.valueOf(val1) + "-" + String.valueOf(val2));
	}
*/
/*
	public static float computeUnaryScore(double[] unaryFeatArr, WeightVector wv, int j) {
		float re = 0;
		for (int i = 0; i < unaryFeatArr.length; i++) {
			if (unaryFeatArr[i] != 0) {
				re += wv.get(singleLetterFeatLen * j + i + 1);
			}
		}
		return re;
	}
	...
	// from i to j
	public static float computeTransitionScore(WeightVector wv, int i, int j) {
		float re = 0;
		re += wv.get(SingleLetterFeatLen * HwOutput.ALPHABET.length + HwFeaturizer.computePairIdx(i, j) + 1);
		return re;
	}
*/	
