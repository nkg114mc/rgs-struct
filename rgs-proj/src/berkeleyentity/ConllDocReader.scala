...
//  }
  ...
  def loadRawConllDocsWithSuffix(path: String, size: Int, suffix: String, lang: Language = Language.ENGLISH, betterParsesFile: String = ""): Seq[ConllDoc] = {
    ...
    val files = rawFiles.filter(file => (file.getAbsolutePath.endsWith(suffix) && !file.getName.contains(".DS_Store")));
  ...
  /*
  def loadRawConllDocsWithFileList(fileList: Array[String], path: String, size: Int, suffix: String, lang: Language = Language.ENGLISH, betterParsesFile: String = ""): Seq[ConllDoc] = {
    Logger.logss("Loading " + size + " docs from " + path + " ending with " + suffix);
    val rawDir = new File(path);
    if (!rawDir.exists() || !rawDir.canRead() || rawDir.listFiles == null || rawDir.listFiles.isEmpty) {
      throw new RuntimeException("Couldn't find directory " + path);
    }
    val rawFilesAll = rawDir.listFiles.sortBy(_.getAbsolutePath());
    val rawFilesList = new ArrayBuffer[String];
    for () {
