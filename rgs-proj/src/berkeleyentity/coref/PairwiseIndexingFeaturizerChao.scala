 ...
 */
@SerialVersionUID(1L)
class PairwiseIndexingFeaturizerChao(val featureIndexer: Indexer[String],
  ...
  val conjTypeIndexer1 = new Indexer[String]()
  val conjTypeIndexer2 = new Indexer[String]()
  
  val featFireCount = new Counter[Int]()
    ...
    */
    if (true) {
      ...
      if (validPair && validTemplate) {
        ...
        conjTypeIndexer1.getIndex(currConjunction)
        conjTypeIndexer2.getIndex(prevConjunction)
        val pairConjunction = currConjunction + prevConjunction
        maybeAddFeat(feats, featName + currConjunction + prevConjunction, addToFeaturizer);
        //maybeAddFeat(feats, pairConjunction, addToFeaturizer);
...
//              Logger.logss(currHead + " " + antHead + ": " + currHeadCount + " " + antHeadCount + " " + pairCount + " " + logBinnedCountUnnorm + " " + logBinnedCountNorm)
    ...
    for (featIdx <- feats) {
      featFireCount.incrementCount(featIdx, 1)
    }
...
object PairwiseIndexingFeaturizerChao {
  val UnkFeatName = "UNK_FEAT";
  
  def printFeatFireCount(featIndexer: Indexer[String],
                         featFireCntr: Counter[Int]) {
    val counter = new Counter[Int]()
    val cntMap = featFireCntr.getEntrySet()
    for (en <- cntMap.asScala) {
      counter.incrementCount(en.getValue.toInt, 1)
    }
    
    val fireCnts = counter.keySet().asScala.toList
    val sorted = fireCnts.sortWith(_ > _)
    //for (cc <- counter.getEntrySet.asScala) {
    //  println("Number of features fire " + cc.getKey + " times: " + cc.getValue)
    //}
    for (c2 <- sorted) {
      println("Number of features fire " + c2 + " times: " + counter.getCount(c2).toInt)
    }
  }
}
