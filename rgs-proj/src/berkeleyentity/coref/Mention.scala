...
class Mention(val rawDoc: ConllDoc,
  ...
  var cachedMentionTypeGold: MentionType = null;
...
//      }
    ...
    // MENTION TYPE
    val goldMentType: MentionType = getGoldMEntType(nerString);
    val predMentType: MentionType = propertyComputer.predictAceMentionType(rawDoc.words(sentIdx), rawDoc.pos(sentIdx), rawDoc.trees(sentIdx), startIdx, headIdx, endIdx, nerString);//goldMentType;
    //var mentionType: MentionType = goldMentType;//predMentType;
    var mentionType: MentionType = if (goldMentType != null) {
      goldMentType;
    } else {
    	if (endIdx - startIdx == 1 && PronounDictionary.isDemonstrative(rawDoc.words(sentIdx)(headIdx))) {
    ...
    val m = new Mention(rawDoc, mentIdx, sentIdx, startIdx, endIdx, headIdx, allHeadIndices, isCoordinated, mentionType, nerString, number, gender);
    m.cachedMentionTypeGold = goldMentType;
    return m;
  }
  
  def getGoldMEntType(nerStr: String) = {
    val tpstr = NerTesting.getGoldMentTyp(nerStr).toLowerCase();
    val tp = if (tpstr.contains("nam")) {
      MentionType.PROPER;
    } else if (tpstr.contains("nom")) {
      MentionType.NOMINAL;
    } else if (tpstr.contains("pro")) {
      MentionType.PRONOMINAL;
    } else if (tpstr.equals("-")) {
      null;
    } else {
      throw new RuntimeException("Unknown ment type str: " + nerStr + " " + tpstr);
    }
    tp;
  }
}
