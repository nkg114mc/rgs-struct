...
object CorefPruner {
  ...
  def buildPruner(strategy: String): CorefPruner = {
    buildPrunerArguments(strategy, Driver.trainPath, Driver.trainSize);
  }
  
  def buildPrunerArguments(strategy: String, trainFolder: String, trainSize: Int): CorefPruner = {
    ...
    if (splitStrategy(0) == "distance") {
      ...
      val docIDsAndModels = trainAndSaveKFoldModels(trainFolder, trainSize, numFolds, modelPath);//(Driver.trainPath, Driver.trainSize, numFolds, modelPath);
  ...
  def trainKFoldModels(trainPath: String, trainSize: Int, numFolds: Int) = {
    ...
    //val allTrainDocs = CorefSystem.loadCorefDocs(trainPath, trainSize, "", Some(numberGenderComputer)).sortBy(_.rawDoc.uid);
