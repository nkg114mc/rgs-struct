...
//    writer.println("#end document");
  ...
  def writeIncompleteConllDocNestedNERJustWiki(writer: PrintWriter,
                                       docName: String,
                                       partNo: Int,
                                       words: Seq[Seq[String]],
                                       pos: Seq[Seq[String]],
    ...
    val corefBits = getCorefBits(sentLens, corefChunks);
    val maybeWikiBits = wikiChunks.map(chunks => getNerBitsPossiblyNestedChunks(sentLens, wikiChunks.get))
    writeIncompleteConllDocFromBitsJustWiki(writer, docName, partNo, words, pos, parseBits, speakers, nerBits, corefBits, maybeWikiBits);
  }
  ...
  def writeIncompleteConllDocFromBits(writer: PrintWriter,
    ...
    def writeIncompleteConllDocFromBitsJustWiki(writer: PrintWriter,
                                      docName: String,
                                      partNo: Int,
                                      words: Seq[Seq[String]],
                                      pos: Seq[Seq[String]],
