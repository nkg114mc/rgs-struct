    ...
    for (corefDoc <- corefDocs) {
      ...
      for (i <- 0 until corefDoc.predMentions.size) {
        ...
        if (!ment.mentionType.isClosedClass()) {
          ...
          val goldLabel = getGoldWikificationUnique(goldWikification(docName), ment);
          if (goldLabel.size >= 1) {
...
//            if (correctIndices.isEmpty && 
  ...
  def extractExamples2(corefDocs: Seq[CorefDoc], goldWikification: CorpusWikiAnnots, wikiDB: WikipediaInterface, filterImpossible: Boolean = false) = {
    val writer = new PrintWriter("wiki_men.log");
    val exs = new ArrayBuffer[JointQueryDenotationExample];
    var numImpossible = 0;
    // Go through all mentions in all documents
...
*/
  ...
  def main(args: Array[String]) {
    ...
    val trainExs = extractExamples2(trainCorefDocs, goldWikification, wikiDB, filterImpossible = true)
    ...
    //val weights = gt.trainAdagrad(trainExs, computer, featIndexer.size, Driver.eta.toFloat, Driver.reg.toFloat, Driver.batchSize, Driver.numItrs);
    ...
    val testExs = extractExamples2(testCorefDocs, goldWikification, wikiDB, filterImpossible = false);
