 ...
 * java -cp /path/to/jar -Xmx8g berkeleyentity.wiki.WikipediaInterface \
...
//    val queries = corefDocs.flatMap(_.predMentions.filter(!_.mentionType.isClosedClass)).flatMap(ment => WikipediaTitleGivenSurfaceDB.extractQueries(ment, ment.headIdx)).toSet;
    ...
    //WikipediaInterface.outputPath = "wiki/output-file.ser.gz";
    var outputPth = "wiki/output-file.ser.gz";
    var categPth = "wiki/categ-file.ser.gz";
    GUtil.save(interface, outputPth);
    if (categPth != "") {
      GUtil.save(interface.categoryDB, categPth);
    }
