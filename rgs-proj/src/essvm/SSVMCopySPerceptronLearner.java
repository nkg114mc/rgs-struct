...
public class SSVMCopySPerceptronLearner extends Learner {

	private static Logger log = LoggerFactory.getLogger(SSVMCopySPerceptronLearner.class);
	 ...
	 * @param params
	 */
	public SSVMCopySPerceptronLearner(AbstractInferenceSolver infSolver, AbstractFeatureGenerator fg,
			SLParameters params) {
	...
	public WeightVector train(SLProblem problem, WeightVector init)
		...
		String fdr = "../CacheWeights";
		checkArffFolder(fdr);
		String rndId = String.valueOf((new Random()).nextInt(Integer.MAX_VALUE));
		...
		while (!done) {
			...
			log.info("Time spend on iteration: " + timr2.getElpseTime());
			...
			//String fileNmAvg = fdr + "/" + "perc" + rndId + "_iter" + String.valueOf(epoch) + ".avg";
			String fileNmAvg = fdr + "/" + "perc" + rndId  + ".avg";
			UtilFunctions.saveObj(avg, fileNmAvg);
			String fileNmW = fdr + "/" + "perc" + rndId + ".w";
			UtilFunctions.saveObj(w, fileNmW);
	...
	protected int doOneIteration(WeightVector w, WeightVector avg,
		...
		for (int exampleId = 0; exampleId < numExamples; exampleId++) {
			...
			////////////
			if ((exampleId > 0) && (exampleId % 10000 == 0)) {
				log.info("Done examples {}", exampleId);
			}
		}
		return count;
	}
	...
	protected void update(IInstance ins, IStructure gold, IStructure prediction,
		...
		//log.info("Feature sparsity: {} and {}", ((SparseFeatureVector)goldFeatures).getNumActiveFeatures(), ((SparseFeatureVector)predictedFeatures).getNumActiveFeatures());

		IFeatureVector update = featureGenerator.getFeatureVectorDiff(ins, gold, prediction);// goldFeatures.difference(predictedFeatures);
