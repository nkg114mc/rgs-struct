...
public class ImageSegFactorGraph extends AbstractFactorGraph {
	...
	private ImageSegFeaturizer featurizer;
	...
	public ImageSegFactorGraph(AbstractInstance insti, AbstractFeaturizer ftzri) {
		init((ImageInstance)insti, (ImageSegFeaturizer)ftzri);
	}
	
	public ImageSegFactorGraph(ImageInstance inst, ImageSegFeaturizer ftzr) {
		init(inst, ftzr);
	}
	
	private void init(ImageInstance inst, ImageSegFeaturizer ftzr) {
	...
	public double computeScoreWithTable(double[] weights, HwOutput output) {
				...
				for (int jdx = 0; jdx < neigbours.length; jdx++) {
					...
					if (segIdx >= 0) { // not a "void" pixel
						...
						if (i < segIdx) { // no repeat pair
							String pairfn = ImageSegFeaturizer.getPairwiseFeatName(alphabet, output.getOutput(i),output.getOutput(segIdx));
		...
		if (featurizer.considerGlobal) {
			for (int jval = 0; jval < alphabet.length; jval++) {
				...
				int idx1 = featurizer.getIndex(ImageSegFeaturizer.getGlobalFeatName(alphabet, jval, 1));
				score += (cnt1 * weights[idx1]);
				int idx0 = featurizer.getIndex(ImageSegFeaturizer.getGlobalFeatName(alphabet, jval, 0));
	...
	public double computeScoreDiffWithTable(double[] weights, SearchAction action, HwOutput output) {
		...
		if (featurizer.considerGlobal) {
			double sc1 = 0;
			// old value
			int idxOld1 = featurizer.getGlobalIndex(oldv, 1);//featurizer.getIndex(ImageSegFeaturizer.getGlobalFeatName(alphabet, oldv, 1)); // 1 -> 0
			sc1 -= weights[idxOld1];
			int idxOld0 = featurizer.getGlobalIndex(oldv, 0);//featurizer.getIndex(ImageSegFeaturizer.getGlobalFeatName(alphabet, oldv, 0));
			sc1 += weights[idxOld0];
			// new value
			int idxNew1 = featurizer.getGlobalIndex(newv, 1);//featurizer.getIndex(ImageSegFeaturizer.getGlobalFeatName(alphabet, newv, 1)); // 0 -> 1
			sc1 += weights[idxNew1];
			int idxNew0 = featurizer.getGlobalIndex(newv, 0);//featurizer.getIndex(ImageSegFeaturizer.getGlobalFeatName(alphabet, newv, 0));
			sc1 -= weights[idxNew0];
			scoreDiff += sc1;
		}
